...
class TransferLearningTagger(BaseTagger):
	...
	def __init__(self, settings, model_name='vgg', feature_extract=True):
	...
	def _init_model(self):
		...
		self.model, self.input_size = initialize_model(model_name=self.model_name, num_classes=len(tasks),
	...
	def _init_optimizer(self):
		...
		self.optimizer = Adam(params_to_update)
		...
		with torch.no_grad():
			...
			for i, batch in enumerate(dataloader):
				if (i + 1) % 10 == 0 or (i+1) == len(dataloader):
					print('Progress Predict {}: [{}/{}]'.format(info, i + 1, len(dataloader)))
